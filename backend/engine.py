...
Flux2Klein uses Qwen3 as its text encoder. encode_prompt() returns
(prompt_embeds, text_ids) — the text_ids are positional coordinates
regenerated internally by the pipeline, so only prompt_embeds is
...
class LatentEngine:
    ...
    def __init__(self, device="cuda", dtype=torch.bfloat16, seed=42):
        ...
        self.anchor_embeddings = []     # list of prompt_embeds tensors
    ...
    def encode_prompt(self, prompt: str) -> torch.Tensor:
        """
        Encode a single prompt → prompt_embeds tensor.

        Flux2Klein uses Qwen3; encode_prompt returns (prompt_embeds, text_ids).
        We only need prompt_embeds for semantic navigation — text_ids are
        positional coordinates regenerated by the pipeline internally.
        """
        with torch.inference_mode():
            prompt_embeds, _text_ids = self.pipe.encode_prompt(prompt)
    ...
    def encode_all(self, base_prompt: str, axis_pairs: list[dict]):
        ...
        prompt_embeds = self.encode_prompt(base_prompt)
    ...
    def encode_anchors(self, anchor_data: list[dict]):
        ...
        for i, anchor in enumerate(anchor_data):
            ...
            self.anchor_embeddings.append(prompt_embeds)
